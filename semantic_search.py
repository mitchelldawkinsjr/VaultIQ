...
class SemanticSearchEngine:
    ...
    def __init__(
        ...
        if not SEMANTIC_SEARCH_AVAILABLE:
            logger.warning(
                "Semantic search dependencies not available. Install sentence-transformers and faiss-cpu."
            )
            return
    ...
    def build_index(self, video_segments: List[Dict]):
        ...
        logger.info(
            f"Building semantic search index for {len(video_segments)} segments"
        )
        ...
        logger.info(
            f"FAISS index built with {self.index.ntotal} segments, dimension {dimension}"
        )
    ...
    def _load_index(self):
        ...
        try:
            ...
            logger.info(
                f"Index exists: {index_path.exists()}, Metadata exists: {metadata_path.exists()}"
            )
            ...
            if index_path.exists() and metadata_path.exists():
                ...
                self.is_initialized = True
                logger.info(
                    f"✅ Loaded search index with {len(self.segments_metadata)} segments"
                )
                return True
            else:
    ...
    def semantic_search(self, query: str, top_k: int = 20) -> List[SearchResult]:
        ...
        if not self.is_initialized:
            ...
            logger.info(
                "Search engine not initialized, attempting to load cached index..."
            )
    ...
    def hybrid_search(
        ...
        logger.info(
            f"Hybrid search found {len(final_results)} combined results for: '{query}'"
        )
