    ...
    for filename, should_be_valid in test_files:
        ...
        try:
            ...
            if is_valid == should_be_valid:
                ...
                status = "✗"
            print(
                f"  {status} {filename}: {is_valid} ({'expected' if is_valid == should_be_valid else 'unexpected'})"
            )
        finally:
            os.unlink(tmp_path)
...
def test_with_sample_video():
    ...
    test_video_paths = ["test_data/test.mp4", "sample.mp4", "test.mp4"]
    ...
    try:
        ...
        if audio_result.is_successful:
            ...
            if (
                audio_result.output_file_path
                and Path(audio_result.output_file_path).exists()
            ):
        ...
        if transcription.is_successful:
            ...
            print(
                f"  Segments: {len(transcription.text_segments) if transcription.text_segments else 0}"
            )
            if transcription.transcribed_text:
                preview = (
                    transcription.transcribed_text[:100] + "..."
                    if len(transcription.transcribed_text) > 100
                    else transcription.transcribed_text
                )
        ...
        if summary:
            ...
            print(
                f"  Transcription available: {'Yes' if summary['transcription'] else 'No'}"
            )
