...
try:
    from ai_enhanced_search import get_ai_engine, enhance_search_with_ai, create_ai_config
    AI_AVAILABLE = True
...
def search_videos(request):
    ...
    try:
        if search_mode == 'semantic' and search_engine.is_initialized:
            ...
            search_results = convert_semantic_results_to_display_format(semantic_results)
            
        elif search_mode == 'hybrid' and search_engine.is_initialized:
            # Hybrid search (combines semantic + keyword)
            video_segments_data = get_video_segments_for_search()
            hybrid_results = search_engine.hybrid_search(query, video_segments_data, top_k=50)
    ...
    return render(request, 'video_processor/library.html', {
        'videos': VideoJob.objects.filter(status=JobStatus.COMPLETED).order_by('-created_at'),
...
def upload_video(request):
    ...
    try:
        ...
        messages.success(request, f'Video "{video_file.name}" uploaded and queued for processing')
...
def handle_youtube_upload(request, youtube_url):
    ...
    try:
        ...
        if not is_youtube_url(youtube_url):
            messages.error(request, 'Invalid YouTube URL. Please provide a valid YouTube link.')
            return redirect('video_library')
        
        # Set up download directory
        media_videos_dir = settings.MEDIA_ROOT / 'videos'
        
        messages.info(request, 'Downloading YouTube video... This may take a few minutes.')
        
        # Download video
        success, video_path, video_info, error_msg = download_youtube_video(youtube_url, media_videos_dir)
        ...
        messages.success(request, f'YouTube video "{video_info["title"]}" downloaded and queued for processing')
    ...
    try:
        ...
        if result.get('processing_errors'):
            ...
            if has_text and has_segments:
                ...
                logger.info(f"Job {job_id} marked as COMPLETED - ready for searching with {len(transcription_data.get('text_segments', []))} segments")
            else:
                job.status = JobStatus.FAILED
                job.error_message = "Transcription incomplete - missing text or segments"
                logger.warning(f"Job {job_id} failed completion check - has_text: {has_text}, has_segments: {has_segments}")
        ...
        is_youtube = (
            job.video_name.startswith(('YouTube_', 'yt_')) or 
            'media/videos/' in job.video_path and any(
                char in job.video_path for char in ['_', '-']
        ...
        if job.status == JobStatus.COMPLETED and is_youtube:
            try:
                ...
                if video_path.exists():
                    ...
                    video_path.unlink()
                    logger.info(f"Cleaned up YouTube video file: {job.video_name} ({file_size_mb:.1f}MB freed)")
    ...
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if duration > 3600:  # 1 hour limit
                return False, None, None, f"Video too long: {duration//60:.1f} minutes (max 60 minutes)"
...
def api_latest_job(request):
    ...
    try:
        ...
        job_info = {
            ...
            'segment_count': len(latest_job.text_segments) if latest_job.text_segments else 0,
            'word_count': latest_job.word_count,
            'language': latest_job.language,
            'error_message': latest_job.error_message,
            'is_searchable': latest_job.status == JobStatus.COMPLETED and bool(latest_job.transcription_text) and bool(latest_job.text_segments)
        }
    ...
    try:
        ...
        video_details = {
            ...
            'metadata': {
                ...
                'width_pixels': job.metadata.get('width_pixels', 0) if job.metadata else 0,
                'height_pixels': job.metadata.get('height_pixels', 0) if job.metadata else 0,
                'frames_per_second': job.metadata.get('frames_per_second', 0) if job.metadata else 0,
                'file_size_bytes': job.metadata.get('file_size_bytes', 0) if job.metadata else 0,
            },
            'transcription': {
                'text': job.transcription_text,
                'language': job.language,
                'word_count': job.word_count,
                'confidence_score': job.transcription.get('confidence_score', 0) if job.transcription else 0,
                'processing_duration_seconds': job.transcription.get('processing_duration_seconds', 0) if job.transcription else 0
            },
            'segments': segments,
...
def api_rebuild_search_index(request):
    ...
    try:
        ...
        videos = VideoJob.objects.filter(status=JobStatus.COMPLETED, transcription__isnull=False)
        
        if not videos.exists():
            return JsonResponse({'error': 'No completed videos found to index'}, status=400)
        ...
        if not all_segments:
            return JsonResponse({'error': 'No text segments found to index'}, status=400)
...
def api_process_job(request):
    ...
    try:
        ...
        if job.status != JobStatus.PENDING:
            return JsonResponse({'error': f'Job {job_id} is not pending (status: {job.status})'}, status=400)
...
def api_timestamp_content(request, job_id):
    ...
    try:
        ...
        if job.status != JobStatus.COMPLETED or not job.transcription:
            return JsonResponse({'error': 'Video not processed or transcription not available'}, status=400)
        ...
        if not current_segment:
            return JsonResponse({'error': f'No content found at timestamp {timestamp}s'}, status=404)
...
def api_detailed_stats(request):
    ...
    try:
        ...
        completed_jobs = VideoJob.objects.filter(status=JobStatus.COMPLETED, processing_time__isnull=False)
        total_processing_time = sum(job.processing_time for job in completed_jobs)
        avg_processing_time = total_processing_time / completed_jobs.count() if completed_jobs.count() > 0 else 0
        
        # Content stats
        total_words = sum(job.word_count or 0 for job in VideoJob.objects.filter(status=JobStatus.COMPLETED))
        total_duration = sum(job.duration_seconds or 0 for job in VideoJob.objects.filter(status=JobStatus.COMPLETED))
        ...
        # Storage stats
        total_file_size = sum(job.file_size_bytes or 0 for job in VideoJob.objects.all())
    ...
    try:
        ...
        for job in pending_jobs:
            jobs_data.append({
                ...
                'video_name': job.video_name,
                'created_at': job.created_at.isoformat(),
                'file_size_mb': round((job.file_size_bytes or 0) / (1024 * 1024), 1) if job.file_size_bytes else 0
            })
...
def serve_video_file(request, video_id):
    ...
    try:
        ...
        if video.is_youtube_video():
            return JsonResponse({'error': 'This is a YouTube video - use the YouTube URL instead'}, status=400)
...
def api_ai_enhanced_search(request):
    ...
    try:
        ...
        logger.info(f"AI Enhanced search: '{query}' (mode: {search_mode}, openai: {use_openai})")
        ...
        if AI_AVAILABLE:
            try:
                ...
                logger.warning(f"AI config check failed: {e} - falling back to regular search")
                ai_available = False
        ...
        try:
            if search_mode == 'semantic' and search_engine.is_initialized:
                ...
                video_segments_data = get_video_segments_for_search()
                hybrid_results = search_engine.hybrid_search(query, video_segments_data, top_k=20)
                search_results = convert_semantic_results_to_display_format(hybrid_results)
            else:
                search_results = perform_keyword_search(query)
        ...
        if not ai_available:
            ...
            # Track search query
            VideoSearchQuery.objects.create(query=query, results_count=len(enhanced_api_results))
        ...
        return JsonResponse({
            ...
            'ai_provider': 'openai' if use_openai else 'huggingface' if ai_enhanced_count > 0 else None
        })
    ...
    try:
        ...
        try:
            if search_engine.is_initialized:
                video_segments_data = get_video_segments_for_search()
                context_results = search_engine.hybrid_search(question, video_segments_data, top_k=context_limit)
                context_data = convert_semantic_results_to_display_format(context_results)
            else:
...
def api_ai_status(request):
    ...
    try:
        ...
        return JsonResponse({
            ...
            'status': 'AI tokens available - full functionality' if config.has_any_tokens else 'No AI tokens - fallback to regular search'
        })
