...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        # Submit command
        submit_parser = subparsers.add_parser('submit', help='Submit a video for processing')
        ...
        # Search command
        search_parser = subparsers.add_parser('search', help='Search video transcriptions')
        search_parser.add_argument('query', help='Search query')
        
        # Timestamp command
        timestamp_parser = subparsers.add_parser('timestamp', help='Get content at specific timestamp')
        timestamp_parser.add_argument('job_id', help='Job ID')
        timestamp_parser.add_argument('timestamp', type=float, help='Timestamp in seconds')
        ...
        rebuild_index_parser = subparsers.add_parser('rebuild-index', help='Rebuild semantic search index')
        search_stats_parser = subparsers.add_parser('search-stats', help='Show search engine statistics')
        ai_search_parser = subparsers.add_parser('ai-search', help='Perform AI-powered semantic search')
        ai_search_parser.add_argument('query', help='Natural language search query')
        ai_search_parser.add_argument('--mode', choices=['semantic', 'hybrid'], default='hybrid',
                                    help='Search mode (semantic or hybrid)')
        
        # Storage management commands
        cleanup_parser = subparsers.add_parser('cleanup-youtube', help='Clean up YouTube video files to save storage')
        cleanup_parser.add_argument('--dry-run', action='store_true', 
                                  help='Show what would be deleted without actually deleting')
        
        # AI Enhancement commands
        ai_status_parser = subparsers.add_parser('ai-status', help='Check AI system status and configuration')
        
        ai_enhanced_search_parser = subparsers.add_parser('ai-enhanced-search', help='Perform AI-enhanced search with Q/A pairs and topic classification')
        ai_enhanced_search_parser.add_argument('query', help='Search query')
        ai_enhanced_search_parser.add_argument('--use-openai', action='store_true', default=False,
                                             help='Use OpenAI instead of Hugging Face')
        ai_enhanced_search_parser.add_argument('--mode', choices=['semantic', 'hybrid'], default='hybrid',
                                             help='Search mode')
        
        ai_qa_parser = subparsers.add_parser('ai-qa', help='Ask a question about your videos using AI')
        ai_qa_parser.add_argument('question', help='Question to ask')
        ai_qa_parser.add_argument('--context-limit', type=int, default=3,
    ...
    def handle_list(self, options):
        ...
        for job in jobs:
            ...
            self.stdout.write(f"   Created: {job.created_at.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def handle_search(self, options):
        ...
        for video, segments in results:
            ...
            for segment in segments:
                self.stdout.write(f"   ⏰ {segment['start_time']:.1f}s - {segment['end_time']:.1f}s:")
    ...
    def handle_timestamp(self, options):
        ...
        if matching_segment:
            ...
            self.stdout.write("=" * 50)
            self.stdout.write(f"Segment: {matching_segment['start']:.1f}s - {matching_segment['end']:.1f}s")
    ...
    def handle_ai_search(self, options):
        ...
        if not search_engine.is_initialized:
            self.stdout.write("❌ Semantic search not available. Run 'rebuild-index' first.")
            return
        ...
        try:
            ...
            for video_data in video_groups.values():
                ...
                for result in segments:
                    ...
                    self.stdout.write(f"   ⏰ {result.start_time:.1f}s - {result.end_time:.1f}s "
    ...
    def handle_cleanup_youtube(self, options):
        ...
        youtube_jobs = VideoJob.objects.filter(
            status=JobStatus.COMPLETED
        ).exclude(video_path__startswith='[CLEANED_UP]')
        ...
        for job in youtube_jobs:
            ...
            is_youtube = (
                'media/videos/' in job.video_path and 
                any(char in job.video_path for char in ['_', '-']) and
                len(Path(job.video_path).stem.split('_')[0]) == 11  # YouTube ID length
            )
    ...
    def handle_ai_status(self, options):
        ...
        try:
            ...
            # API Configuration
            self.stdout.write("\n🔑 API Configuration:")
            self.stdout.write(f"   OpenAI API Key: {'✅ Configured' if config.openai_api_key else '❌ Not configured'}")
            self.stdout.write(f"   Hugging Face Token: {'✅ Configured' if config.huggingface_api_key else '❌ Not configured'}")
            
            if not config.openai_api_key and not config.huggingface_api_key:
                self.stdout.write("\n❌ No API keys configured!")
                self.stdout.write("   Please check AI_SETUP_INSTRUCTIONS.md for setup details.")
                return
            ...
            if config.huggingface_api_key:
                ...
                self.stdout.write(f"   HF Summarization: {config.hf_summarization_model}")
            
            # Engine Status
            self.stdout.write("\n🔧 Engine Status:")
            self.stdout.write(f"   AI Engine: {'✅ Initialized' if ai_engine else '❌ Not initialized'}")
            ...
            if ai_engine:
                ...
                self.stdout.write("\n💡 Test Commands:")
                self.stdout.write("   python manage.py video_manager ai-enhanced-search 'relationship advice'")
                self.stdout.write("   python manage.py video_manager ai-qa 'How to build confidence?'")
            
        except ImportError:
            self.stdout.write("\n❌ AI Enhancement System Not Available")
            self.stdout.write("   Install dependencies: pip install aiohttp openai python-dotenv")
        except Exception as e:
    ...
    def handle_ai_enhanced_search(self, options):
        ...
        try:
            from ai_enhanced_search import get_ai_engine, enhance_search_with_ai
            import asyncio
            
            ai_engine = get_ai_engine()
            if not ai_engine:
                self.stdout.write("❌ AI engine not available. Run 'ai-status' to check configuration.")
                return
            ...
            self.stdout.write(f"   AI Provider: {'OpenAI' if use_openai else 'Hugging Face'}")
            self.stdout.write("=" * 60)
            
            # Get regular search results first
            from video_processor.views import perform_keyword_search, get_video_segments_for_search, convert_semantic_results_to_display_format
            ...
            self.stdout.write(f"🤖 Applying AI enhancements to {len(ai_input_results)} results...")
            ...
            for i, result in enumerate(enhanced_results, 1):
                ...
                if result.confidence_score:
                    self.stdout.write(f"   🎯 AI Confidence: {result.confidence_score:.2f}")
            
        except ImportError:
            self.stdout.write("❌ AI features not available. Install dependencies first.")
        except Exception as e:
    ...
    def handle_ai_qa(self, options):
        ...
        try:
            ...
            for video in videos:
                if any(word.lower() in video.transcription_text.lower() for word in question.split()):
                    segments = video.search_segments(' '.join(question.split()[:3]))  # Use first 3 words
            ...
            if answer:
                ...
                for i, context in enumerate(context_data, 1):
                    self.stdout.write(f"   {i}. {context['title']} (at {context['timestamp']:.1f}s)")
            else:
                self.stdout.write("❌ Could not generate an answer. Try rephrasing your question.")
