...
from typing import Dict, Any, Optional, Tuple, Protocol, List
...
class AudioProcessor(Protocol):
    ...
    def extract_audio_from_video(self, video_path: str, output_path: Optional[str] = None) -> ProcessingResult:
...
class TranscriptionService(Protocol):
    ...
    def transcribe_audio_file(self, audio_path: str, language: str = "auto") -> TranscriptionResult:
...
class VideoFileValidator:
    ...
    def __init__(self, max_file_size_bytes: int = MAX_FILE_SIZE_BYTES):
        ...
        logger.debug(f"VideoFileValidator initialized with {max_file_size_bytes / (1024*1024):.1f}MB limit")
    ...
    def validate_video_file(self, file_path: str) -> Tuple[bool, Optional[str]]:
        ...
        if file_extension not in self.SUPPORTED_FORMATS:
            ...
            return False, f"Unsupported video format '{file_extension}'. Supported: {supported_formats_list}"
        ...
        if file_size_bytes > self.max_file_size_bytes:
            ...
            return False, f"File too large: {file_size_mb:.1f}MB (maximum: {max_size_mb:.1f}MB)"
...
class VideoMetadataExtractor:
    ...
    def __init__(self):
        ...
        logger.info(f"MetadataExtractor initialized - OpenCV: {self.opencv_available}, FFmpeg: {self.ffmpeg_available}")
    ...
    def extract_metadata(self, file_path: str) -> Optional[VideoMetadata]:
        ...
        try:
            ...
            if self.opencv_available:
                ...
                logger.warning("No video analysis tools available - using basic file metadata only")
    ...
    def _extract_metadata_with_opencv(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            duration_seconds = total_frame_count / frames_per_second if frames_per_second > 0 else 0.0
            
            video_capture.release()
            
            logger.debug(f"OpenCV metadata extracted: {width_pixels}x{height_pixels}, {frames_per_second}fps, {duration_seconds:.2f}s")
    ...
    def _extract_metadata_with_ffmpeg(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"FFmpeg metadata extracted: {width_pixels}x{height_pixels}, {frames_per_second}fps, {duration_seconds:.2f}s")
...
class AudioExtractor:
    ...
    def __init__(self, temp_directory: Optional[str] = None):
        ...
        logger.debug(f"AudioExtractor initialized with temp directory: {self.temp_directory}")
    ...
    def extract_audio_from_video(self, video_path: str, output_path: Optional[str] = None) -> ProcessingResult:
        ...
        try:
            ...
            if output_path is None:
                ...
                output_path = os.path.join(self.temp_directory, f"{video_filename}_extracted_audio.mp3")
            ...
            logger.info(f"Audio extraction successful in {processing_duration:.2f} seconds")
...
class WhisperTranscriptionService:
    ...
    def transcribe_audio_file(self, audio_path: str, language: str = "auto") -> TranscriptionResult:
        ...
        try:
            ...
            transcription_result = self.whisper_model.transcribe(audio_path, **transcription_options)
            ...
            if 'segments' in transcription_result and transcription_result['segments']:
                ...
                confidence_score = sum(segment_confidences) / len(segment_confidences) if segment_confidences else None
...
class CoreVideoProcessor:
    ...
    def _log_initialization_status(self):
        ...
        logger.info(f"  - Metadata extraction: {'OpenCV' if OPENCV_AVAILABLE else 'FFmpeg/Basic'}")
        logger.info(f"  - Audio extraction: {'FFmpeg' if self.audio_extractor.ffmpeg_available else 'Not available'}")
        logger.info(f"  - Transcription: {'Whisper' if self.transcription_service.whisper_available else 'Not available'}")
    ...
    def extract_audio_from_video(self, video_file_path: str, audio_output_path: Optional[str] = None) -> ProcessingResult:
        """
        ...
        return self.audio_extractor.extract_audio_from_video(video_file_path, audio_output_path)
    
    def transcribe_audio_file(self, audio_file_path: str, language_code: str = "auto") -> TranscriptionResult:
        """
        Transcribe an audio file to text.
        ...
        """
        return self.transcription_service.transcribe_audio_file(audio_file_path, language_code)
    
    def transcribe_video_file(self, video_file_path: str, language_code: str = "auto") -> TranscriptionResult:
        """
        ...
        try:
            ...
            transcription_result = self.transcribe_audio_file(
                audio_extraction_result.output_file_path, 
                language_code
            )
            
            # Step 3: Clean up temporary audio file if configured
            if self.cleanup_temporary_files and audio_extraction_result.output_file_path:
                try:
                    os.remove(audio_extraction_result.output_file_path)
                    logger.debug(f"Cleaned up temporary audio file: {audio_extraction_result.output_file_path}")
                except Exception as cleanup_error:
                    logger.warning(f"Could not clean up temporary audio file: {cleanup_error}")
    ...
    def create_comprehensive_video_summary(self, video_file_path: str, language_code: str = "auto") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if video_metadata:
                ...
                logger.info(f"Metadata extracted: {video_metadata.resolution_string}, {video_metadata.duration_seconds:.2f}s")
            else:
                analysis_summary['processing_errors'].append("Failed to extract video metadata")
            
            # Generate transcription if service is available
            if self.transcription_service.whisper_available:
                transcription_result = self.transcribe_video_file(video_file_path, language_code)
                ...
                if transcription_result.is_successful:
                    ...
                    logger.info(f"Transcription completed: {transcription_result.word_count} words in {transcription_result.detected_language}")
                else:
                    analysis_summary['processing_errors'].append(
                        f"Transcription failed: {transcription_result.error_message}"
                    )
            else:
                analysis_summary['processing_errors'].append("Transcription service not available")
            
            logger.info(f"Video analysis completed with {len(analysis_summary['processing_errors'])} errors")
...
Examples:
  ...
  python core_video_processor.py summary video.mp4 --output results.json
    ...
    if args.action == 'metadata':
        ...
        summary = processor.create_comprehensive_video_summary(args.video_file, args.language)
