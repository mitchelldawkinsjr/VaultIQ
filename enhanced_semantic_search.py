...
class EnhancedSemanticSearchEngine:
    ...
    def is_initialized(self) -> bool:
        ...
        return (
            self.model is not None and self.index is not None and len(self.segments) > 0
        )
    ...
    def create_enhanced_chunks(
        self, text: str, video_id: str, video_title: str, start_time: float = 0
    ) -> List[Dict[str, Any]]:
        """
    ...
    def build_enhanced_index(self, video_segments: List[Dict[str, Any]]):
        ...
        build_time = time.time() - start_time
        logger.info(
            f"✅ Enhanced index built with {len(all_chunks)} segments in {build_time:.2f}s"
        )
    ...
    def search(
        ...
        try:
            ...
            for i, (similarity, idx) in enumerate(zip(similarities[0], indices[0])):
                ...
                if filter_topics:
                    if not any(
                        topic in segment.get("topics", []) for topic in filter_topics
                    ):
                        continue
            ...
            logger.info(
                f"Enhanced search completed: {len(results)} results in {search_time:.3f}s"
            )
            return results
