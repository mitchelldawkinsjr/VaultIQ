...
from enhanced_semantic_search import get_enhanced_search_engine, EnhancedSearchResult
...
class RAGQuestionAnswering:
    ...
    def answer_question(self, 
                       ...
                       filter_topics: Optional[List[str]] = None) -> QAResult:
        """
        ...
        try:
            ...
            result = QAResult(
                ...
                metadata={
                    ...
                    "avg_source_confidence": sum(r.confidence_score for r in search_results) / len(search_results),
                    "topics_found": list(set(topic for r in search_results for topic in r.topic_tags)),
            ...
            logger.info(f"Question answered in {processing_time:.2f}s using {method} method")
            return result
    ...
    def _prepare_context(self, search_results: List[EnhancedSearchResult]) -> str:
        ...
        for result in search_results:
            ...
            if (result.context_window and 
                result.context_window != result.segment_text and
                len(result.context_window) > len(result.segment_text)):
    ...
    def _choose_qa_method(self, question: str, context: str) -> str:
        ...
        extractive_keywords = ['who', 'what', 'when', 'where', 'which', 'how many', 'what is']
        
        # Generative QA is good for explanatory questions
        generative_keywords = ['why', 'how', 'explain', 'describe', 'compare', 'summarize']
    ...
    def _generate_answer(self, question: str, context: str, method: str) -> Tuple[str, float]:
        """
    ...
    def _hybrid_qa(self, question: str, context: str) -> Tuple[str, float]:
        ...
        extractive_answer, extractive_confidence = self._extractive_qa(question, context)
        ...
        generative_answer, generative_confidence = self._generative_qa(question, context)
    ...
    def _update_stats(self, result: QAResult):
        ...
        if result.sources:
            self.stats['retrieval_success_rate'] = (
                (self.stats['retrieval_success_rate'] * (self.stats['total_questions'] - 1) + 1) /
                self.stats['total_questions']
            )
        
        self.stats['average_processing_time'] = (
            (self.stats['average_processing_time'] * (self.stats['total_questions'] - 1) + 
             result.processing_time) / self.stats['total_questions']
        )
