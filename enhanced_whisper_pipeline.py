...
class EnhancedWhisperPipeline:
    ...
    def __init__(self, 
        ...
        logger.info(f"Enhanced Whisper pipeline initialized: {model_name} on {self.device}")
    ...
    def transcribe_with_enhancement(self, 
                                  audio_path: str,
        ...
        try:
            ...
            for i, segment in enumerate(result["segments"]):
                ...
                confidence = self._calculate_confidence(segment.get("avg_logprob", -1.0))
            ...
            confidence_stats = {
                ...
                "median": np.median(segment_confidences) if segment_confidences else 0.0,
    ...
    def enhance_transcript_quality(self, 
                                 segments: List[TranscriptionSegment],
                                 remove_filler_words: bool = True,
                                 fix_capitalization: bool = True,
                                 merge_short_segments: bool = True) -> List[TranscriptionSegment]:
        """
    ...
    def _remove_filler_words(self, segments: List[TranscriptionSegment]) -> List[TranscriptionSegment]:
    ...
    def _fix_capitalization(self, segments: List[TranscriptionSegment]) -> List[TranscriptionSegment]:
        ...
        for segment in segments:
            ...
            import re
            segment.text = re.sub(r'([.!?]\s+)([a-z])', 
                                lambda m: m.group(1) + m.group(2).upper(), 
                                segment.text)
        
        return segments
    
    def _merge_short_segments(self, segments: List[TranscriptionSegment], 
                            min_duration: float = 2.0) -> List[TranscriptionSegment]:
        ...
        for next_segment in segments[1:]:
            ...
            if duration < min_duration and next_segment.start - current_segment.end < 1.0:
    ...
    def _export_srt(self, result: TranscriptionResult, output_path: Optional[str] = None) -> str:
