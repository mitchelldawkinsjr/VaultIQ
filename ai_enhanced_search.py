...
class AIEnhancedSearchEngine:
    ...
    def __init__(self, config: AIConfig):
        ...
        if config.has_any_tokens:
            ...
            logger.info("No AI tokens found - will fall back to regular search functionality")
    ...
    async def _call_openai_api(self, prompt: str, max_tokens: int = None) -> Optional[str]:
    ...
    async def classify_topic_huggingface(self, content: str) -> str:
        ...
        result = await self._call_huggingface_api(self.config.hf_classification_model, inputs)
    ...
    async def analyze_sentiment_huggingface(self, content: str) -> float:
        ...
        result = await self._call_huggingface_api(self.config.hf_sentiment_model, inputs)
    ...
    async def summarize_content_huggingface(self, content: str) -> str:
        ...
        result = await self._call_huggingface_api(self.config.hf_summarization_model, inputs)
        if result and isinstance(result, list) and result:
    ...
    async def enhance_search_results(self, search_results: List[Dict], use_openai: bool = True) -> List[EnhancedSearchResult]:
        ...
        if not self.is_ai_available:
            ...
            for result in search_results:
                enhanced = EnhancedSearchResult(
                    ...
                    ai_enhanced=False,
                    confidence_score=result.get('relevance_score', 0)  # Use original relevance as confidence
                )
        ...
        for result in search_results:
            try:
                ...
                if use_openai and self.config.has_openai_token:
                    ...
                    enhanced.generated_questions = results[0] if not isinstance(results[0], Exception) else []
                    enhanced.topic_classification = results[1] if not isinstance(results[1], Exception) else "general"
                    enhanced.summary = results[2] if not isinstance(results[2], Exception) else ""
                    enhanced.key_concepts = results[3] if not isinstance(results[3], Exception) else []
                    ...
                    enhanced.generated_questions = results[0] if not isinstance(results[0], Exception) else []
                    enhanced.topic_classification = results[1] if not isinstance(results[1], Exception) else "general"
                    enhanced.sentiment_score = results[2] if not isinstance(results[2], Exception) else 0.0
                    enhanced.summary = results[3] if not isinstance(results[3], Exception) else ""
                
                # Calculate confidence score based on available data
                confidence_factors = []
                if enhanced.generated_questions:
                    confidence_factors.append(0.3)
                if enhanced.topic_classification and enhanced.topic_classification != "general":
                    confidence_factors.append(0.3)
                if enhanced.summary:
    ...
    async def answer_question(self, question: str, context_results: List[Dict]) -> Optional[str]:
...
def create_ai_config() -> AIConfig:
    ...
    return AIConfig(
        ...
        hf_classification_model=os.getenv('HF_CLASSIFICATION_MODEL', 'facebook/bart-large-mnli'),
...
def get_ai_engine() -> Optional[AIEnhancedSearchEngine]:
    ...
    if _ai_engine is None:
        ...
        if not config.has_any_tokens:
            logger.info("AI engine created without tokens - will fall back to regular search")
    return _ai_engine

async def enhance_search_with_ai(search_results: List[Dict], use_openai: bool = True) -> List[EnhancedSearchResult]:
